...
public class DateValueSelectCaloriePopupWin extends PopupWindow implements OnClickListener{
	...
	public DateValueSelectCaloriePopupWin (Context context, View v,int select) {
		super(context);
		ctx = context;
		LayoutInflater inflater = (LayoutInflater) ctx
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mMenuView = inflater.inflate(R.layout.task_value_select_for_calarie_goal, null);
		btnBack = (Button) mMenuView.findViewById(R.id.goal_value_select_calorie_back);
		btnSave = (Button) mMenuView.findViewById(R.id.goal_value_select_calorie_save);
		tvDisplaySelect = (TextView) mMenuView.findViewById(R.id.goal_value_select_calorie_display);
		btnBack.setOnClickListener(this);
		btnSave.setOnClickListener(this);
		
		
		 initWheelH(ctx,R.id.calorie_select_value_in_wheel_h);
		 initWheelL(ctx,R.id.calorie_select_value_in_wheel_l);
...
//		btnCncel.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {  
		case R.id.goal_value_select_calorie_back:  
			dismiss();
			break;
		case R.id.goal_value_select_calorie_save:
			if(hValue == 0&& lValue == 0)
			{	
				new AlertDialog.Builder(ctx).setTitle(R.string.not_allowed_data).setMessage(R.string.calorie_can_not_be_0).setPositiveButton(R.string.ok, null).show(); 
	    		return;
			}
			else
			{
				returnValue = (hValue*10+lValue)*10;
...
//            break;  
	   ...
	   {
		   tvDisplaySelect.setText(""+(hValue*10+lValue)*10);
	   }
		...
		public void storeGoalInfoInDB(int value)
		{
           ...
           cv.put(DBForTreatmill.HISTORY_CALORIE_OR_DISTANCE, DBForTreatmill.HISTORY_CALORIE);
